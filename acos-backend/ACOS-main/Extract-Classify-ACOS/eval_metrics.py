...
def pred_eval(_e, args, logger, tokenizer, model, dataloader, eval_gold, label_list, device, task_name, eval_type='valid'):
    ...
    if eval_type == 'valid':
        pipeline_file = cs.open(args.output_dir+os.sep+'valid.txt', 'w', encoding='utf-8')
    else:
        pipeline_file = cs.open(args.output_dir+os.sep+'pred4pipeline.txt', 'w', encoding='utf-8')
    for text in preds:
...
def pair_eval(_e, args, logger, tokenizer, model, dataloader, gold, label_list, device, task_name, eval_type='valid'):
    ...
    if eval_type == 'valid':
        ...
        pipeline_res = cs.open(args.output_dir+os.sep+'result.txt', 'w', encoding='utf-8')
        for key in golds:
