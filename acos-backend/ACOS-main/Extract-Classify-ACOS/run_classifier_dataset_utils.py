...
class QuadProcessor(DataProcessor):
    ...
    def get_dev_examples(self, data_dir, domain_type):
        """See base class."""
        file_path = os.path.join(data_dir, domain_type + '_quad_bert.tsv')
        logger.info("LOOKING AT %s",file_path)
        return self._create_examples(
...
class CategorySentiProcessor(DataProcessor):
    ...
    def get_dev_examples(self, data_dir, domain_type):
        """See base class."""
        file_path = os.path.join(data_dir, domain_type + '_pair_1st.tsv')
        logger.info("LOOKING AT %s",file_path)
        return self._create_examples(
            self._read_tsv(file_path), "dev")
...
def convert_examples_to_features(examples, label_list, max_seq_length,
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        if ' @@@ ' not in example.text_a:
            logger.warning(f"Skipping line, ' @@@ ' separator not found in: {example.text_a}")
            continue
        _product_id, text_part = example.text_a.strip().split(' @@@ ', 1)

        if not text_part:
            logger.warning(f"Skipping line, empty text for product_id: {_product_id}")
            continue
        orig_tokens = tokenizer.tokenize(text_part)
...
def convert_examples_to_features2nd(examples, label_list, max_seq_length,
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        text_with_id, ao_tags = example.text_a.strip().split('####', 1)

        if ' @@@ ' not in text_with_id:
            logger.warning(f"Skipping line, ' @@@ ' separator not found in: {text_with_id}")
            continue

        _product_id, text_part = text_with_id.split(' @@@ ', 1)
